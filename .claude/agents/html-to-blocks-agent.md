...
description: Converts raw HTML mockups (pasted, file, URL, or folder) into Gutenberg block markup for WordPress. Use when the user invokes /html-to-blocks, asks to "turn this HTML into blocks", or needs batch HTML→Gutenberg conversion via WP-CLI. Inspired by 10up/convert-to-blocks, adapted as a Claude agent that emits canonical block-comment serialization.
